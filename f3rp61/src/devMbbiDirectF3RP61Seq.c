...
static long init_record(mbbiDirectRecord *precord)
{
    ...
    //
    const int ret = f3rp61seqParseLink(plink, dpvt, kRead, kWord, (dbCommon *)precord, "devMbbiDirectF3RP61Seq");
    if (ret < 0) {
    ...
    if (option == 'W') {        // Dummy option for Word access
        ...
        errlogPrintf("devMbbiDirectF3RP61Seq: %s : unsupported option \'%c\'\n", precord->name, option);
        precord->pact = 1;
        return -1;
    }
